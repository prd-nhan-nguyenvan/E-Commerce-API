...
@pytest.mark.django_db
def test_update_item_from_cart_remove_success(
    api_client, user, product, add_product_to_cart
):
    api_client.force_authenticate(user=user)
    data = {"quantity": 0}
    response = api_client.patch(add_product_to_cart, data, format="json")
...
@pytest.mark.django_db
def test_empty_cart_success(api_client, user, product, add_product_to_cart):
    api_client.force_authenticate(user=user)
    response = api_client.delete(reverse("empty-cart"))
    assert response.status_code == status.HTTP_204_NO_CONTENT
